...
module datapath (
   ...
   Multiplier mult (.start(), .reset(reset_L), .clock(clock), 
                    .a(aluSrcA), .b(aluSrcB), .done(), .ZN_flags(multCC), .out())

   // Result selection between alu or multiplication coprocessor
   logic [15:0] Result;
   Mux2to1 outSel (.I0(aluResult), .I1(multResult), .Y(Result), .S())

   // Condition code selection between alu or multiplication coprocessor
   logic [3:0] CC;
   Mux2to1 outSel (.I0(newCC), .I1({multCC, 2'b00}), .Y(CC), .S())
